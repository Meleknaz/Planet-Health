...
class _MyAppState extends State<MyApp> {
  ...
  Future _showDialog(BuildContext context) async {
    return showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text("Analiz edilecek kaynağı seç! "),
            content: SingleChildScrollView(
              child: ListBody(
                children: <Widget>[
                  GestureDetector(
                    child: Text("Galeri' den seç "),
                    onTap: () {
  ...
  Widget build(BuildContext context) {
    ...
    stackChildren.add(Positioned(
      ...
      left: 0.0,
      width: size.width,
      child: _image == null ? Text('Analiz için sağ alt butondan bitkini seç.',
        textAlign: TextAlign.center,
        overflow: TextOverflow.ellipsis,
        style: const TextStyle(fontWeight: FontWeight.bold,fontSize: 22.2),) : Image.file(_image),
    ));
...
//        width: size.width,
        child: RaisedButton(
          ...
          child: Text('Hastalık için çözüm'),
          color: Colors.blue,
        ),
      ));
    }
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('TOAF AI Hastalık tespiti'),
      ),
      body: Stack(
